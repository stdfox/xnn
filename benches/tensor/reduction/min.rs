//! Min reduction operation benchmarks.
...
pub(crate) fn bench_min_reduce(c: &mut Criterion) {
    let ctx = Context::new().unwrap();
    let mut group = configure(c, "tensor/min_reduce");
...
pub(crate) fn bench_min_reduce_axis0(c: &mut Criterion) {
    let ctx = Context::new().unwrap();
    let mut group = configure(c, "tensor/min_reduce/axis0");
...
pub(crate) fn bench_min_reduce_axis1(c: &mut Criterion) {
    let ctx = Context::new().unwrap();
    let mut group = configure(c, "tensor/min_reduce/axis1");
