//! Mean reduction operation benchmarks.
...
pub(crate) fn bench_mean_reduce(c: &mut Criterion) {
    let ctx = Context::new().unwrap();
    let mut group = configure(c, "tensor/mean_reduce");
...
pub(crate) fn bench_mean_reduce_axis0(c: &mut Criterion) {
    let ctx = Context::new().unwrap();
    let mut group = configure(c, "tensor/mean_reduce/axis0");
...
pub(crate) fn bench_mean_reduce_axis1(c: &mut Criterion) {
    let ctx = Context::new().unwrap();
    let mut group = configure(c, "tensor/mean_reduce/axis1");
