...
fn test_exp_2d() {
    let ctx = Context::try_default().unwrap();
    let data = vec![0.0, 1.0, -1.0, 2.0, -2.0, 0.0];
...
fn test_exp_large() {
    ...
    for val in &result.to_vec().unwrap() {
        assert_relative_eq!(*val, E, epsilon = 1e-4);
    }
}
