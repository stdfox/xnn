//! Tests for `Tensor::elu` operation.

use approx::assert_relative_eq;
use xnn::{Context, Tensor};

fn elu_ref(x: f32, alpha: f32) -> f32 {
    if x < 0.0 { alpha * (x.exp() - 1.0) } else { x }
}
...
fn test_elu_basic() {
    ...
    let expected: Vec<f32> = data.iter().map(|&x| elu_ref(x, 1.0)).collect();
...
fn test_elu_custom_alpha() {
    ...
    let t = Tensor::<f32>::from_slice(&ctx, &data).unwrap();
    let alpha = 0.5;
    let result = t.elu(Some(alpha)).unwrap();
    let out = result.to_vec().unwrap();
    let expected: Vec<f32> = data.iter().map(|&x| elu_ref(x, alpha)).collect();
    for (a, b) in out.iter().zip(expected.iter()) {
        assert_relative_eq!(a, b, epsilon = 1e-4);
    }
}
