...
#[test]
fn test_gt_scalar_broadcast_reverse() {
    let ctx = Context::try_default().unwrap();
...
#[test]
fn test_gt_incompatible_shapes() {
    let ctx = Context::try_default().unwrap();
...
#[test]
fn test_gt_scalar_to_scalar() {
    let ctx = Context::try_default().unwrap();
    let a = Tensor::<f32>::constant(&ctx, &[], &[15.0]).unwrap();
    let b = Tensor::<f32>::constant(&ctx, &[], &[SCALAR]).unwrap();
    let c = a.gt(&b).unwrap();
