...
#[test]
fn test_ne_scalar_broadcast_reverse() {
    let ctx = Context::try_default().unwrap();
...
#[test]
fn test_ne_incompatible_shapes() {
    let ctx = Context::try_default().unwrap();
