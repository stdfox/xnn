...
/// Kernel marker type.
struct Select<T, U>(PhantomData<(T, U)>);

/// Kernel trait implementation.
impl<T: LogicalElement, U: NumericElement> Kernel for Select<T, U> {
    ...
    fn wgsl() -> String {
        ...
        alloc::format!(
            r"
                ...
                @group(0) @binding(1) var<storage, read> a: array<{val_ty}>;
                @group(0) @binding(2) var<storage, read> b: array<{val_ty}>;
                @group(0) @binding(3) var<storage, read_write> y: array<{val_ty}>;
                ...
                fn main(@builtin(global_invocation_id) gid: vec3<u32>) {{
                    ...
                    y[tid] = select(b[b_idx], a[a_idx], x[x_idx] != {zero});
                }}
            "
        )
    }
...
pub(crate) fn execute<T: LogicalElement, U: NumericElement>(
    ctx: &Context,
    x: &Buffer<T>,
    a: &Buffer<U>,
    b: &Buffer<U>,
    ...
    let byte_size = (y.len() * U::NATIVE_SIZE) as u64;
