//! Element-wise scalar addition kernel.
//!
//! Adds a scalar to each element of a buffer using a compute shader.

use crate::kernel::{debug_assert_len, debug_assert_same_device, debug_assert_same_len};
use crate::{Buffer, Element, GpuContext};

/// Workgroup size for the `add_scalar` kernel.
const WORKGROUP_SIZE: u32 = 256;

/// Maximum workgroups per dimension (WebGPU limit).
const MAX_WORKGROUPS_PER_DIM: u32 = 65535;

/// Adds a scalar to each element of a buffer.
///
...
pub fn add_scalar<T: Element>(ctx: &GpuContext, a: &Buffer<T>, b: &Buffer<T>, c: &Buffer<T>) {
...
fn create_pipeline<T: Element>(device: &wgpu::Device) -> wgpu::ComputePipeline {
    ...
    let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("add_scalar_shader"),
        source: wgpu::ShaderSource::Wgsl(shader_source.into()),
    });

    device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        label: Some("add_scalar_pipeline"),
        layout: None,
        module: &shader,
...
mod tests {
    ...
    fn test_add_scalar() {
        ...
        assert_eq!(ctx.read_buffer(&c).unwrap(), vec![43.0, 44.0, 45.0, 46.0]);
    ...
    fn test_add_scalar_assert_same_len() {
        let ctx = GpuContext::default();
