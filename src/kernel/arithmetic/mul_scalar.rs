...
pub fn mul_scalar<T: Element>(ctx: &GpuContext, a: &Buffer<T>, b: &Buffer<T>, c: &Buffer<T>) {
...
fn create_pipeline<T: Element>(device: &wgpu::Device) -> wgpu::ComputePipeline {
    ...
    let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("mul_scalar_shader"),
        source: wgpu::ShaderSource::Wgsl(shader_source.into()),
    });

    device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        label: Some("mul_scalar_pipeline"),
        layout: None,
        module: &shader,
...
mod tests {
    ...
    fn test_mul_scalar() {
        ...
        mul_scalar(&ctx, &a, &b, &c);
        assert_eq!(ctx.read_buffer(&c).unwrap(), vec![42.0, 84.0, 126.0, 168.0]);
        ...
        mul_scalar(&ctx, &a, &b, &c);
        assert_eq!(ctx.read_buffer(&c).unwrap(), vec![42, 84, 126, 168]);
        ...
        let result = ctx.read_buffer(&c).unwrap();
        for val in &result {
            assert_relative_eq!(*val, 2.0 * PI, epsilon = 1e-5);
        }
    ...
    fn test_mul_scalar_assert_same_len() {
        let ctx = GpuContext::default();
