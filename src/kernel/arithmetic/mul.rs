...
mod tests {
    ...
    fn test_mul() {
        ...
        mul(&ctx, &a, &b, &c).unwrap();
        assert_eq!(ctx.read_buffer(&c).unwrap(), vec![2.0, 6.0, 12.0, 20.0]);
        ...
        mul(&ctx, &a, &b, &c).unwrap();
        assert_eq!(ctx.read_buffer(&c).unwrap(), vec![2, 6, 12, 20]);
        ...
        mul(&ctx, &a, &b, &c).unwrap();
        assert_eq!(ctx.read_buffer(&c).unwrap(), vec![6.0; 42]);
