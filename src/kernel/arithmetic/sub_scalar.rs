//! Element-wise scalar subtraction kernel.
//!
...
/// Subtracts a scalar from each element of a buffer.
///
...
pub fn sub_scalar<T: Element>(ctx: &Context, a: &Buffer<T>, b: &Buffer<T>, c: &Buffer<T>) {
...
fn create_pipeline<T: Element>(device: &wgpu::Device) -> wgpu::ComputePipeline {
    ...
    let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("sub_scalar_shader"),
        source: wgpu::ShaderSource::Wgsl(shader_source.into()),
    });

    device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        label: Some("sub_scalar_pipeline"),
        layout: None,
        module: &shader,
...
mod tests {
    ...
    fn test_sub_scalar() {
        ...
        // f32
        let a = ctx
            .create_buffer_from_slice(&[43.0f32, 44.0, 45.0, 46.0])
            .unwrap();
