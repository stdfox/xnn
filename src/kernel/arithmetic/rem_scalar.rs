//! Element-wise scalar remainder kernel.
//!
//! Computes the remainder of division by a scalar using a compute shader.

use crate::kernel::{assert_len, assert_same_len, debug_assert_same_device};
use crate::{Buffer, Element, Error, GpuContext};

/// Workgroup size for the `rem_scalar` kernel.
const WORKGROUP_SIZE: u32 = 256;

/// Maximum workgroups per dimension (WebGPU limit).
const MAX_WORKGROUPS_PER_DIM: u32 = 65535;

/// Computes the remainder of division by a scalar.
///
...
fn create_pipeline<T: Element>(device: &wgpu::Device) -> wgpu::ComputePipeline {
    ...
    let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("rem_scalar_shader"),
        source: wgpu::ShaderSource::Wgsl(shader_source.into()),
    });

    device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        label: Some("rem_scalar_pipeline"),
        layout: None,
        module: &shader,
...
mod tests {
    ...
    fn test_rem_scalar() {
        ...
        rem_scalar(&ctx, &a, &b, &c).unwrap();
        let result = ctx.read_buffer(&c).unwrap();
        assert_relative_eq!(result[0], 5.5 % PI, epsilon = 1e-5);
        assert_relative_eq!(result[1], 7.0 % PI, epsilon = 1e-5);
        assert_relative_eq!(result[2], 10.0 % PI, epsilon = 1e-5);
        assert_relative_eq!(result[3], 3.0 % PI, epsilon = 1e-5);

        // i32
        let a = ctx.create_buffer_from_slice(&[7i32, 10, 15, 86]).unwrap();
        ...
        // u32
        let a = ctx.create_buffer_from_slice(&[7u32, 10, 15, 86]).unwrap();
    ...
    #[test]
    fn test_rem_scalar_length_mismatch() {
        let ctx = GpuContext::default();
        ...
        let result = rem_scalar(&ctx, &a, &b, &c);
        assert!(result.is_err());
    }

    #[test]
    fn test_rem_scalar_wrong_b_length() {
        let ctx = GpuContext::default();
