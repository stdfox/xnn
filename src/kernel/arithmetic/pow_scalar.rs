//! Element-wise scalar power kernel.
//!
...
/// Raises each element of a buffer to a scalar power.
///
...
fn create_pipeline<T: FloatElement>(device: &wgpu::Device) -> wgpu::ComputePipeline {
    ...
    let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("pow_scalar_shader"),
        source: wgpu::ShaderSource::Wgsl(shader_source.into()),
    });

    device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        label: Some("pow_scalar_pipeline"),
        layout: None,
        module: &shader,
...
mod tests {
    ...
    fn test_pow_scalar() {
        ...
        assert_relative_eq!(result[2], 16.0, epsilon = 1e-5);
        assert_relative_eq!(result[3], 25.0, epsilon = 1e-5);
