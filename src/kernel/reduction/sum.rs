...
/// Sum reduction kernel marker.
pub(crate) struct SumReduce<T>(PhantomData<T>);
...
impl<T: NumericElement> Kernel for SumReduce<T> {
    const LABEL: &'static str = "sum_reduce";
    type Output = T;
...
/// Executes sum reduction kernel along specified axes.
///
/// # Panics
///
