//! Round operation for tensor buffers.
...
const LABEL: &str = "round";

/// Marker type for pipeline caching.
struct Round<T>(core::marker::PhantomData<T>);

/// Generates WGSL shader for round operation.
fn shader() -> String {
    format!(
        r"
...
pub(crate) fn round<T: FloatElement>(
    ...
    let pipeline = ctx.get_or_create_pipeline(TypeId::of::<Round<T>>(), shader, LABEL)?;
