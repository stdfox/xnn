//! Tangent operation for tensor buffers.
...
struct Tan<T>(core::marker::PhantomData<T>);

/// Generates WGSL shader for tan operation.
fn shader() -> String {
    format!(
        r"
...
pub(crate) fn tan<T: FloatElement>(
    ctx: &Context,
    a: &Buffer<T>,
    b: &Buffer<T>,
) -> Result<(), Error> {
    ...
    let pipeline = ctx.get_or_create_pipeline(TypeId::of::<Tan<T>>(), shader, LABEL)?;
