//! Cosine operation for tensor buffers.
...
struct Cos<T>(core::marker::PhantomData<T>);

/// Generates WGSL shader for cos operation.
fn shader() -> String {
    format!(
        r"
...
pub(crate) fn cos<T: FloatElement>(
    ctx: &Context,
    a: &Buffer<T>,
    b: &Buffer<T>,
) -> Result<(), Error> {
    ...
    let pipeline = ctx.get_or_create_pipeline(TypeId::of::<Cos<T>>(), shader, LABEL)?;
