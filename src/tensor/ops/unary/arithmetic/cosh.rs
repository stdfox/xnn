//! Hyperbolic cosine operation for tensor buffers.
...
const LABEL: &str = "cosh";

/// Marker type for pipeline caching.
struct Cosh<T>(core::marker::PhantomData<T>);

/// Generates WGSL shader for cosh operation.
fn shader() -> String {
    format!(
        r"
...
pub(crate) fn cosh<T: FloatElement>(
    ctx: &Context,
    a: &Buffer<T>,
    b: &Buffer<T>,
) -> Result<(), Error> {
    ...
    let pipeline = ctx.get_or_create_pipeline(TypeId::of::<Cosh<T>>(), shader, LABEL)?;
