//! Sine operation for tensor buffers.
...
struct Sin<T>(core::marker::PhantomData<T>);

/// Generates WGSL shader for sin operation.
fn shader() -> String {
    format!(
        r"
...
pub(crate) fn sin<T: FloatElement>(
    ctx: &Context,
    a: &Buffer<T>,
    b: &Buffer<T>,
) -> Result<(), Error> {
    ...
    let pipeline = ctx.get_or_create_pipeline(TypeId::of::<Sin<T>>(), shader, LABEL)?;
