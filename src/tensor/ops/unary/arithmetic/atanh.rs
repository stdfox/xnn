//! Inverse hyperbolic tangent operation for tensor buffers.
...
const LABEL: &str = "atanh";

/// Marker type for pipeline caching.
struct Atanh<T>(core::marker::PhantomData<T>);

/// Generates WGSL shader for atanh operation.
fn shader() -> String {
    format!(
        r"
...
pub(crate) fn atanh<T: FloatElement>(
    ctx: &Context,
    a: &Buffer<T>,
    b: &Buffer<T>,
) -> Result<(), Error> {
    ...
    let pipeline = ctx.get_or_create_pipeline(TypeId::of::<Atanh<T>>(), shader, LABEL)?;
