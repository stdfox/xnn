//! Hyperbolic sine operation for tensor buffers.
...
const LABEL: &str = "sinh";

/// Marker type for pipeline caching.
struct Sinh<T>(core::marker::PhantomData<T>);

/// Generates WGSL shader for sinh operation.
fn shader() -> String {
    format!(
        r"
...
pub(crate) fn sinh<T: FloatElement>(
    ctx: &Context,
    a: &Buffer<T>,
    b: &Buffer<T>,
) -> Result<(), Error> {
    ...
    let pipeline = ctx.get_or_create_pipeline(TypeId::of::<Sinh<T>>(), shader, LABEL)?;
