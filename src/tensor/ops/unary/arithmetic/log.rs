//! Natural logarithm operation for tensor buffers.
...
struct Log<T>(core::marker::PhantomData<T>);

/// Generates WGSL shader for log operation.
fn shader() -> String {
    format!(
        r"
...
pub(crate) fn log<T: FloatElement>(
    ctx: &Context,
    a: &Buffer<T>,
    b: &Buffer<T>,
) -> Result<(), Error> {
    ...
    let pipeline = ctx.get_or_create_pipeline(TypeId::of::<Log<T>>(), shader, LABEL)?;
