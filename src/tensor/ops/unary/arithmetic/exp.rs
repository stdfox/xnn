//! Exponential operation for tensor buffers.
...
struct Exp<T>(core::marker::PhantomData<T>);

/// Generates WGSL shader for exp operation.
fn shader() -> String {
    format!(
        r"
...
pub(crate) fn exp<T: FloatElement>(
    ctx: &Context,
    a: &Buffer<T>,
    b: &Buffer<T>,
) -> Result<(), Error> {
    ...
    let pipeline = ctx.get_or_create_pipeline(TypeId::of::<Exp<T>>(), shader, LABEL)?;
