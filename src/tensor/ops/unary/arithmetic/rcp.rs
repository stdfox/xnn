//! Reciprocal operation for tensor buffers.
...
struct Rcp<T>(core::marker::PhantomData<T>);

/// Generates WGSL shader for rcp operation.
fn shader() -> String {
    format!(
        r"
...
pub(crate) fn rcp<T: FloatElement>(
    ctx: &Context,
    a: &Buffer<T>,
    b: &Buffer<T>,
) -> Result<(), Error> {
    ...
    let pipeline = ctx.get_or_create_pipeline(TypeId::of::<Rcp<T>>(), shader, LABEL)?;
