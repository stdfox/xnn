//! Arc tangent operation for tensor buffers.
...
const LABEL: &str = "atan";

/// Marker type for pipeline caching.
struct Atan<T>(core::marker::PhantomData<T>);

/// Generates WGSL shader for atan operation.
fn shader() -> String {
    format!(
        r"
...
pub(crate) fn atan<T: FloatElement>(
    ctx: &Context,
    a: &Buffer<T>,
    b: &Buffer<T>,
) -> Result<(), Error> {
    ...
    let pipeline = ctx.get_or_create_pipeline(TypeId::of::<Atan<T>>(), shader, LABEL)?;
