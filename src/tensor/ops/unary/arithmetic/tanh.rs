//! Hyperbolic tangent operation for tensor buffers.
...
const LABEL: &str = "tanh";

/// Marker type for pipeline caching.
struct Tanh<T>(core::marker::PhantomData<T>);

/// Generates WGSL shader for tanh operation.
fn shader() -> String {
    format!(
        r"
...
pub(crate) fn tanh<T: FloatElement>(
    ctx: &Context,
    a: &Buffer<T>,
    b: &Buffer<T>,
) -> Result<(), Error> {
    ...
    let pipeline = ctx.get_or_create_pipeline(TypeId::of::<Tanh<T>>(), shader, LABEL)?;
