//! Inverse hyperbolic cosine operation for tensor buffers.
...
const LABEL: &str = "acosh";

/// Marker type for pipeline caching.
struct Acosh<T>(core::marker::PhantomData<T>);

/// Generates WGSL shader for acosh operation.
fn shader() -> String {
    format!(
        r"
...
pub(crate) fn acosh<T: FloatElement>(
    ctx: &Context,
    a: &Buffer<T>,
    b: &Buffer<T>,
) -> Result<(), Error> {
    ...
    let pipeline = ctx.get_or_create_pipeline(TypeId::of::<Acosh<T>>(), shader, LABEL)?;
