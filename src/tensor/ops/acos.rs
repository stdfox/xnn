//! Arc cosine operation for tensor buffers.
...
const LABEL: &str = "acos";

/// Marker type for pipeline caching.
struct Acos<T>(core::marker::PhantomData<T>);

/// Generates WGSL shader for acos operation.
fn shader() -> String {
    format!(
        r"
...
pub(crate) fn acos<T: FloatElement>(
    ctx: &Context,
    a: &Buffer<T>,
    b: &Buffer<T>,
) -> Result<(), Error> {
    ...
    let pipeline = ctx.get_or_create_pipeline(TypeId::of::<Acos<T>>(), shader, LABEL)?;
