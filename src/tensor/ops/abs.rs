//! Absolute value operation for tensor buffers.

use core::any::TypeId;

use crate::{Buffer, Context, Error, NumericElement};
...
struct Abs<T>(core::marker::PhantomData<T>);

/// Generates WGSL shader for absolute value operation.
...
pub(crate) fn abs<T: NumericElement>(
    ctx: &Context,
    a: &Buffer<T>,
    b: &Buffer<T>,
) -> Result<(), Error> {
    ...
    let pipeline = ctx.get_or_create_pipeline(TypeId::of::<Abs<T>>(), shader::<T>, LABEL)?;
