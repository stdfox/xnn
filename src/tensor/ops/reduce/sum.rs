...
const LABEL: &str = "sum_reduce";

/// Sum reduction parameters passed to shader as uniform.
#[repr(C)]
...
struct SumReduceMarker<T>(PhantomData<T>);
    ...
    format!(
        r"
            ...
            fn main(
                ...
                // Compute base coordinates from output index
                ...
                // Each thread handles a portion of the reduction
                var acc: {ty} = {ty}(0);
                ...
                while reduction_idx < params.reduction_len {{
                    ...
                    for (var i = 0u; i < params.rank; i++) {{
                        var coord: u32;
                        if is_reduced(i) {{
                            // Compute coordinate within reduced dimension
                            var red_stride = 1u;
                ...
                if tid == 0u {{
                    ...
                    if params.normalize != 0u {{
                        result = result / {ty}(params.reduction_len);
                    }}
