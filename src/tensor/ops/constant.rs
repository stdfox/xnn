//! Constant fill operation for tensor buffers.
...
struct Constant<T>(core::marker::PhantomData<T>);

/// Generates WGSL shader for constant fill operation.
fn shader<T: Element>() -> String {
    let wgsl_type = T::wgsl_type();
    let vec4_type = format!("vec4<{wgsl_type}>");

    format!(
        r"
            @group(0) @binding(0) var<storage, read_write> buffer: array<{vec4_type}>;
            @group(0) @binding(1) var<uniform> value: {wgsl_type};

            @compute @workgroup_size({WORKGROUP_SIZE})
            fn main(@builtin(global_invocation_id) gid: vec3<u32>) {{
                let tid = gid.x + gid.y * {MAX_WORKGROUPS}u * {WORKGROUP_SIZE}u;
                if tid < arrayLength(&buffer) {{
                    buffer[tid] = {vec4_type}(value);
                }}
            }}
        "
    )
...
pub(crate) fn constant<T: Element>(
    ctx: &Context,
    buffer: &Buffer<T>,
    value: &wgpu::Buffer,
) -> Result<(), Error> {
    ...
    let pipeline = ctx.get_or_create_pipeline(TypeId::of::<Constant<T>>(), shader::<T>, LABEL)?;
