//! Floor operation for tensor buffers.
...
const LABEL: &str = "floor";

/// Marker type for pipeline caching.
struct Floor<T>(core::marker::PhantomData<T>);

/// Generates WGSL shader for floor operation.
fn shader() -> String {
    format!(
        r"
...
pub(crate) fn floor<T: FloatElement>(
    ctx: &Context,
    a: &Buffer<T>,
    b: &Buffer<T>,
) -> Result<(), Error> {
    ...
    let pipeline = ctx.get_or_create_pipeline(TypeId::of::<Floor<T>>(), shader, LABEL)?;
