//! Inverse hyperbolic sine operation for tensor buffers.
...
const LABEL: &str = "asinh";

/// Marker type for pipeline caching.
struct Asinh<T>(core::marker::PhantomData<T>);

/// Generates WGSL shader for asinh operation.
fn shader() -> String {
    format!(
        r"
...
pub(crate) fn asinh<T: FloatElement>(
    ctx: &Context,
    a: &Buffer<T>,
    b: &Buffer<T>,
) -> Result<(), Error> {
    ...
    let pipeline = ctx.get_or_create_pipeline(TypeId::of::<Asinh<T>>(), shader, LABEL)?;
