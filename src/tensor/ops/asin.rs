//! Arc sine operation for tensor buffers.
...
const LABEL: &str = "asin";

/// Marker type for pipeline caching.
struct Asin<T>(core::marker::PhantomData<T>);

/// Generates WGSL shader for asin operation.
fn shader() -> String {
    format!(
        r"
...
pub(crate) fn asin<T: FloatElement>(
    ctx: &Context,
    a: &Buffer<T>,
    b: &Buffer<T>,
) -> Result<(), Error> {
    ...
    let pipeline = ctx.get_or_create_pipeline(TypeId::of::<Asin<T>>(), shader, LABEL)?;
